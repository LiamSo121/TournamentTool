...
class GamesTool:
        ...
        while True:
            try:
                ...
                if 1 <= user_choice <= len(tournament_to_decide):
                    ...
                    self.helper.green_colored_printer(f"You selected: {selected_tournament}")
                    break
                else:
    ...
    def get_item_ids_from_main_games_board(self):
        item_ids = []
        for item in self.main_Board_items:
            item_id = item['id']
            item_columns = item['column_values'] 
            ...
            while True:
                try:
                    ...
                    if 1 <= user_choice <= len(relevant_tournament_rounds):
                        ...
                        self.helper.green_colored_printer(f"You selected: {selected_tournament_round}")
                        return user_choice
                    else:
                        self.helper.red_colored_printer("Invalid choice. Please enter a valid number.")
                except ValueError:
...
game.get_column_id_from_main_games_board()
# fetch data from main games board and store it as variable
game.get_data_from_main_games_board()
#get and choose tournament from main game board
game.get_main_board_tournaments()
# get the wanted round number
game.helper.validate_round_number()
# get relevant items id from main board
game.get_item_ids_from_main_games_board()
print(game.main_board_relevant_items_id)
print(len(game.main_board_relevant_items_id))
#
# Need to work on all other five boards
